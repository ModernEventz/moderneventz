// File: app/vendors/[...id]/page.tsx (or wherever your page is)
"use server";
...
import PhotoGalleryWrapper from "@/components/PhotoGalleryWrapper"; // New import
...
const generateAvatar = (name: any) => {
  ...
  return <Image src={avatarUrl} alt={`${initials} Avatar`} width={50} height={50} />;
};
...
const Page = async ({ params }: ParamsProps) => {
  ...
  const {
    ...
    reviews,
    profile,
  } = await getVendorById({ vendorId: params.id[0] });
        ...
        <div className="mt-8 mb-8 grid gap-8 grid-cols-1 md:grid-cols-[2fr_1fr]">
          <div>
            <div className="my-4">
              ...
              {description}
              <p className="mt-4 text-sm leading-6 col-start-1 sm:col-span-2 lg:mt-6 lg:row-start-4 lg:col-span-1 dark:text-slate-400"></p>
            </div>
            {rating ? (
              <dl className="mt-4 text-xs font-medium flex items-center">
                ...
                <dd className="text-rose-600 flex items-center dark:text-rose-400">
                  ...
                  <span>
                    {rating} <span className="text-slate-400 font-normal">({rating_count})</span>
                  </span>
                </dd>
        ...
        <div className="flex w-full flex-col bg-gray-100">
          ...
          <div>
            <div className="flex flex-row">
              <p className="ml-10 text-lg font-bold">Location:</p>
              <p className="ml-2">{profile[0].location}</p>
            </div>
            <div className="flex flex-row">
              <p className="ml-10 text-lg font-bold">Number:</p>
              <p className="ml-2">
                <span>{0}</span>
